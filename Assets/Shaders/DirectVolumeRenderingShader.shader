...
Shader "VolumeRendering/DirectVolumeRenderingShader"
{
    Properties
    {
        ...
        _MaskTex("Organ Mask Texture", 3D) = ""{}
        _TFTex("Transfer Function Texture (Generated)", 2D) = "" {}
        _Color("Color Transfer", Integer) = -1
        _ContrastCutoff("contrast Cutoff val", Vector) = (.0, 1.0, 0.5, .0)
        _ContrastAdjust("contrast Adjust val", Vector) = (.0, 1.0, 0.5, .0)
        //_MinVal("Min val", Range(0.0, 1.0)) = 0.0
        //_MaxVal("Max val", Range(0.0, 1.0)) = 1.0
        //_Contrast("contrast val", Vector) = (.0, 1.0, 0.5, .0)
        //_u_maskbits("Mask bit set", Integer) = 0
    }
    SubShader
    {
        ...
        Pass
        {
            ...
            #pragma multi_compile __ TFCOLOR_SPECTRUM
            #pragma multi_compile __ CONTRAST_CUTOFF
            #pragma multi_compile __ SHOW_ORGAN
            #pragma multi_compile __ SHOW_BODY
            #pragma multi_compile __ SHOW_ORGAN_RECOLOR
...
//#endif
            ...
            float3 AdjustContrastBrightness(float3 color, float3 contrast) {
                float cr = 1.0 / (contrast.y - contrast.x);
                color = clamp(cr * (color - contrast.x), .0, 1.0);
                return clamp(color + contrast.z * 2.0 - 1.0, .0, 1.0);
            }

            float4 getHelmTFColor(float density, int organ_bit) {
#ifdef MODE_DVR
                if (organ_bit < 0) return .0f;
                float4 sampled_color = tex2Dlod(_TFTex, float4(density, 0.0f, 0.0f, 0.0f));
#elif MODE_MIP
                #ifndef SHOW_BODY
                    if (organ_bit < 0) return .0f;
                #endif
                float4 sampled_color = density;
#else
                if (organ_bit < 0) organ_bit = 0;
                float4 sampled_color = tex2Dlod(_TFTex, float4(density, 0.0f, 0.0f, 0.0f));
#endif

                bool body_in_show_organ = (organ_bit == 0);
#ifndef SHOW_ORGAN
                body_in_show_organ = true;
#endif

                if (body_in_show_organ) {
                    #ifdef TFCOLOR_SPECTRUM
                        sampled_color.xyz = (_Color == 0) ? density : hex2rgb(COLOR_SCHEME_HEX[(_Color - 1) % 4][int(density * 255.0)]);
                    #endif
                }else {
                    #ifdef SHOW_ORGAN_RECOLOR
                        sampled_color = float4(ORGAN_COLORS[organ_bit - 1], density);
                    #elif TFCOLOR_SPECTRUM 
                        sampled_color.xyz = (_Color == 0) ? density : hex2rgb(COLOR_SCHEME_HEX[(_Color - 1) % 4][int(density * 255.0)]);
                    #endif
                }

#if defined(MODE_SURF) || defined(CONTRAST_CUTOFF)
                return sampled_color;
#endif
                return float4(
                    AdjustContrastBrightness(sampled_color.xyz, _ContrastAdjust.xyz),
                    sampled_color.a);
            }

            float4 getHelmTFColor_(float density, int organ_bit) {
#ifdef MODE_DVR
                if (organ_bit < 0) return .0f;
                float4 sampled_color = tex2Dlod(_TFTex, float4(density, 0.0f, 0.0f, 0.0f));
#else
                ...
                if (body_in_show_organ) {
                    ...
                    sampled_color.xyz = density;
#ifdef SHOW_ORGAN_RECOLOR
                    sampled_color = float4(ORGAN_COLORS[organ_bit - 1] * density, density);
#elif TFCOLOR_SPECTRUM 
                    sampled_color.xyz = (_Color == 0) ? density : hex2rgb(COLOR_SCHEME_HEX[(_Color - 1) % 4][int(density * 255.0)]);
#endif
                }
            ...
            int getMaskBit(float3 pos) {
                float value = tex3Dlod(_MaskTex, float4(pos.x, pos.y, pos.z, 0.0f)).r * 255.0;
                if (value == 0) {
#ifdef SHOW_BODY
                    return 0;
#endif
                ...
                //return ((_u_maskbits & uint(1)) == uint(1)) ? 0 : -1;

                uint mask_value = (uint) value;
                int u_organ_num = 7;
                int CHECK_BIT = int(-1);
                
                //check if organ
                for (uint i = uint(0); i < u_organ_num; i++) {
                    //if (((_u_maskbits >> uint(i + uint(1))) & uint(1)) == uint(0)) continue;
            ...
            {
                //TODO
                return tex3Dlod(_GradientTex, float4(pos.x, 1.0f - pos.y, 1.0 - pos.z, 0.0f)).rgb;
            }
...
#ifdef CONTRAST_CUTOFF
                    if (density < _ContrastCutoff.x || density > _ContrastCutoff.y) continue;
#endif
...
#else
                    float4 src = getHelmTFColor(density, getMaskBit(currPos));
#endif
            ...
            frag_out frag_mip(frag_in i)
            {
...
#ifdef CONTRAST_CUTOFF
                    if (density < _ContrastCutoff.x || density > _ContrastCutoff.y) within_range = false;
#endif
                    if (within_range && density > maxDensity)
                    {
                        maxDensity = density;
                ...
                output.colour = getHelmTFColor(maxDensity, getMaskBit(maxDensityPos));
#if DEPTHWRITE_ON
            ...
            frag_out frag_surf(frag_in i)
            {
                ...
                for (int iStep = 0; iStep < raymarchInfo.numSteps; iStep++)
                {
                    ...
                    if (within_range)
                    {
                        ...
                        col = getHelmTFColor(density, getMaskBit(currPos));
